---
title: "AS01_R-Basic"
author: "你是誰 R09342000 新聞所碩五"
date: "2021/02/22"
output:
  html_document:
    number_sections: no
    theme: united
    highlight: tango
    toc: yes
    toc_float:
      collapsed: no
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, results = 'hold', comment = '#>', error = TRUE)
```

## 作業: R-Basic

滿分共 100 分。

### variable (10 分)

請創建 3 個變數，`var_credits`代表你本學期的學分數，`var_perhour`代表你預計平均每學分的每周投入課業的時間，`var_study`用前面兩個變數相乘，代表你本學期每周預計投入課業的總時間，最後請將`var_study`印出。

助教不會根據投入時間評斷你，請放心填答，謝謝。

```{r}
### your code

```

### vector 基礎操作 (15 分)

下方提供 Readr 所開放的[臺灣 COVID-19 確診病例資料](https://github.com/readr-media/readr-data/tree/master/covid-19)，並已經抓出前十位確診者的性別、年齡、狀態。請分別印出這十位患者中

- 第 3 位確診者的性別   
- 第 5 位確診者的年齡   
- 奇數位確診者的狀態

```{r}
df <- read.csv("data/covid19_comfirmed_case_taiwan.csv")
vector_gender <- df$gender[1:10]
vector_age <- df$age[1:10]
vector_state <- df$state[1:10]

### your code

```

### vector 與 data frame (20 分) 

同上方資料 

- 請以 `vector_gender_first10_last10` 為名，利用`[]`，創造出一個包含前十位患者與末十位患者性別的 vector。   
- 請以 `df_first10` 為名，將 `vector_gender`, `vector_age`, `vector_state` 拼成一個 dataframe，並將結果印出。   

```{r}
### your code

```

### lists (20 分) 

下方列出部分新聞所本學期開設課程，包含教師姓氏, 課程名稱, 修課限制人數, 開設單位等。請協助

- 抓出教師姓氏   
- 計算學生人數平均   
- 修改`student_quota`元素，將修課限制人數**加倍**，造成老師困擾  
- 在 list 中插入新元素 `weekday`，內容為 `c("三","二","五","三")`，並將結果印出

```{r}
list_journalism_sample <-
  list(teacher=c("chen","hung","liu","lee"),
       course=c("新聞寫作","數位新聞法規與倫理","採訪寫作二","	新聞編輯與資訊設計"),
       student_quota=c(20,15,9,20),
       dept="journalism")

### your code

```

### data frame (35 分) 

在回到臺灣 COVID-19 確診病例資料。

- 請創建一個名為 `df_first20` 的 dataframe，裡面僅有前 20 位確診者  
- 請印出 `df_first20` 中女性(female)的年齡(age)  
- 請印出 `df_first20` 中編號(patient_id)為奇數者的確診日期(released_date)
- 請印出 `df_first20` 中五十歲以上(Above 50)者女性的的狀態(state)
- 請印出 `df_first20` 中，五十歲以上者**以外**所有人的旅遊史(tourism_history)  


```{r}
df <- read.csv("data/covid19_comfirmed_case_taiwan.csv")
### your code

```

