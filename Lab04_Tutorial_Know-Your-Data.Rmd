...
- AS02 小軒偏食題請參考答案，這裡討論 AS02 星座題
- [範例連結](https://p4css.github.io/R4CSS_TA/AS02_R-Intermediate_ref.html)，共分為三個部分
...
- 安裝或載入遇到問題
 - 非常容易發生，可能是安裝時漏東西，或是本地電腦缺東西
 - `Error：package or namespace load failed for ‘tidyverse’ in loadNamespace(i, c(lib.loc, .libPaths()), versionCheck = vI[[i]]) there is no package called 'dplyr'`
 - 解決方法是看它的 error message 說什麼，上面說缺 `dplyr`
- summary
 - 套件 = 書，安裝套件 = 下載書，載入套件 = 從圖書館借書
 - 安裝只要安裝一次就好，所以安裝完程式碼可以刪掉或註解掉
 - `# install.packages("stringr")`
 - 檔案的最上面先載入你的套件 `library(stringr)`
 - 底下就可以快樂的使用函數 `str_c("a","b")`
...
- 前面提過，檔案編碼可能有問題，可以用 notepad ++ / 記事本 / excel 觀察編碼
